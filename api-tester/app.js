...
(function () {
  ...
  function renderResponse(panelId, status, data, duration) {
    const panel = $(`#${panelId}`);
    const statusClass = status >= 200 && status < 300 ? 'success' : status >= 400 && status < 500 ? 'warning' : 'error';
    const statusText = status >= 200 && status < 300 ? 'OK' : status >= 400 && status < 500 ? 'Client Error' : 'Server Error';
  ...
  async function healthCheck() {
    ...
    try {
      ...
      renderResponse('auth-login-response', result.status, result.data, result.duration);
      if (result.ok) {
  ...
  async function doLogin(email, password) {
    ...
    try {
      ...
      if (result.ok && result.data.data) {
        ...
        updateAuthUI();
        showToast(`Login berhasil sebagai ${state.currentUser.name}`, 'success');
      } else {
  ...
  function initEventListeners() {
    ...
    $('#btn-update-profile').addEventListener('click', async () => {
      ...
      try {
        ...
        renderResponse('auth-update-profile-response', r.status, r.data, r.duration);
    ...
    $('#btn-change-password').addEventListener('click', async () => {
      ...
      try {
        const r = await apiRequest('PUT', '/api/auth/change-password', { body });
    ...
    $('#btn-create-announcement').addEventListener('click', async () => {
      ...
      try {
        const r = await apiRequest('POST', '/api/announcements', { body: formData });
    ...
    $('#btn-update-announcement').addEventListener('click', async () => {
      ...
      try {
        const r = await apiRequest('PUT', `/api/announcements/${id}`, { body: formData });
    ...
    $('#btn-update-event').addEventListener('click', async () => {
      ...
      try {
        const r = await apiRequest('PUT', `/api/events/${id}`, { body: formData });
